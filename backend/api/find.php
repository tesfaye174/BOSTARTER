...
try {
    ...
    echo json_encode([
        'stato' => 'errore',
        'messaggio' => 'Problema interno del server. Riprova pi� tardi.'
    ]);
    exit();
}
...
function gestisciRicercaProgetti($connessioneDb, $logger) {
    try {
        ...
        echo json_encode([
            'stato' => 'errore',
            'messaggio' => 'Si � verificato un problema nella ricerca. Riprova tra qualche momento.',
...
function validaParametriRicerca($parolaChiave, $categoria, $ordinamento) {
    ...
    if (!empty($parolaChiave)) {
        ...
        if (strlen($parolaChiave) > 100) {
            return [
                'valido' => false,
                'errore' => 'La parola chiave � troppo lunga (massimo 100 caratteri)'
            ];
        }
