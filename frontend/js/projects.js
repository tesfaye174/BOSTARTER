...
document.addEventListener('DOMContentLoaded', () => {    // Inizializza configurazione conforme al PDF
    ...
    function displayProjectsCompliant(projects, category) {
        ...
        const categoryConfig = category !== 'all' ? window.getCategoryConfig(category) : null; projectsGrid.innerHTML = projects.map(project => createProjectCardCompliant(project, categoryConfig)).join('');
...
window.BostarterProjects = {
    filter: function (category, status, sort) {
        if (category && !window.isCategoryCompliant(category)) {
            return false;
        }
