            ...
            if (!empty($validation_errors)) {
                ...
                $conn = $db->getConnection();                try {
                    ...
                    $user = $stmt->fetch(PDO::FETCH_ASSOC);
                      if ($user) {
                        // Verifica password - usando solo password_hash come da schema
                        $password_valid = false;
                        
                        if (!empty($user['password_hash'])) {
                            // Verifica con password_verify per hash moderni
                            if (password_verify($password, $user['password_hash'])) {
                                $password_valid = true;
                            }
                            // Fallback per MD5 legacy se la hash è corta (32 caratteri)
                            elseif (strlen($user['password_hash']) === 32 && md5($password) === $user['password_hash']) {
                                $password_valid = true;
                            }                            // Fallback per plain text (solo per debug/test)
