    ...
    try {
        ...
        $stmt = $conn->prepare("
            SELECT p.*, u.nickname as creatore_nickname,
                   ...
                   COUNT(f.id) as numero_sostenitori
            FROM progetti p 
        ...
        if ($project) {
            $stmt = $conn->prepare("
                SELECT f.importo, f.data_finanziamento, u.nickname
...
$days_left = $project ? max(0, floor((strtotime($project["data_scadenza"]) - time()) / (60 * 60 * 24))) : 0;
?>
<!DOCTYPE html>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <div class="navbar-nav">
                    ...
                    <?php else: ?>
                        ...
                        <a class="btn btn-bostarter-primary ms-2" href="auth/register.php">Registrati</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                        ...
                        <div class="card-bostarter sticky-top animate-fade-right" style="top: 120px;">
                            <div class="card-body">
                                ...
                                <?php if ($project["stato"] === "aperto" && $days_left > 0): ?>
                                    <?php if ($is_logged_in): ?>
                                        <button class="btn btn-bostarter-primary w-100 mb-3" data-bs-toggle="modal" data-bs-target="#supportModal">
                                            <i class="fas fa-heart me-2"></i>Sostieni questo progetto
                                        </button>
                                    <?php else: ?>
                                        <a href="auth/login.php" class="btn btn-bostarter-primary w-100 mb-3">
                                            <i class="fas fa-sign-in-alt me-2"></i>Accedi per sostenere
                                        </a>
                                    <?php endif; ?>
                                <?php else: ?>
                                ...
                                <?php endif; ?>
                                <button class="btn btn-outline-primary w-100" onclick="shareProject()">
